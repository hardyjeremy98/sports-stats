...
def test_collect_package_versions_absent_package_is_unknown():
    versions = collect_package_versions(["matchlab-core", "definitely-not-a-real-package-xyz"])
    assert versions["definitely-not-a-real-package-xyz"] == "unknown"
    assert versions["matchlab-core"] != "unknown"  # workspace package is installed


def test_default_package_names_include_expected_set():
    assert set(DEFAULT_PACKAGE_NAMES) == {
        "matchlab-core", "torch", "trackers", "supervision", "inference",
...
def test_yolo_local_provenance_reports_path_and_correct_sha256(tmp_path):
    from matchlab_core.stages.detect.yolo_local import YoloLocalDetector
...
def test_roboflow_detector_provenance_uses_model_id_as_revision_no_weights_hash():
    from matchlab_core.stages.detect.roboflow import RoboflowDetector
...
def test_default_stage_provenance_is_empty_list():
    from matchlab_core.stages.identity.none import NoIdentityResolver
...
def test_face_identity_provenance_reports_pack_without_a_local_path():
    from matchlab_core.stages.identity.face import FaceIdentityResolver
...
def test_siglip_provenance_default_model_reports_verified_apache_license():
    from matchlab_core.stages.team.siglip import SiglipTeamClassifier
...
def test_roboflow_keypoints_provenance_uses_model_id_as_revision():
    from matchlab_core.stages.calibrate.roboflow_keypoints import RoboflowKeypointCalibrator
