...
external calibrator — the real PnLCalib model (HRNet keypoints + lines +
points-and-lines optimization, its own sibling venv, see
docs/reference/external-calibrators-setup.md), or the permissive in-repo
reference calibrator — run as a subprocess via `matchlab_core.calib.bridge`.

This module is the *pipeline* side of the isolation boundary: it never imports
PnLCalib, HRNet, or torch, only the bridge (which itself knows nothing
...
pitch centimeters, top-left origin. See the exchange contract for the required
...
# calibrator CLI. An operator points `command` at the real PnLCalib entrypoint
# (its own sibling venv) to swap in the genuine model — see the eval config
# configs/pipeline.pnlcalib-eval.yaml.
...
class PnLCalibCalibrator(Calibrator):
    ...
    def provenance(self) -> list[ModelProvenance]:
        ...
        return [
            ModelProvenance(
                ...
                license=LicenseAxes(
                    code="GPL-2.0 (PnLCalib, sibling venv, subprocess-isolated)"
                    if kp
    ...
    def _fill_reprojected_keypoints(
        ...
        PnLCalib is line/heatmap based and has no per-frame detected-keypoint set
        of its own, but the Lab's "Pitch keypoints" overlay draws
        `keypoints_image`. Showing the canonical pitch vertices placed on the
        field by the calibration is exactly what that overlay is for — and lets a
        viewer judge calibration quality directly. Vertices that fall well
