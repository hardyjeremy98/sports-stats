...
# `trackers==2.4.0` trackers/core/botsort/tracklet.py (Apache-2.0, unchanged
# license). Modifications: renamed BoTSORTTracklet -> BoTSORTReidTracklet; adds
# a per-track appearance EMA (`smooth_feat` + `update_features`) and widens
# `update(bbox, feat=None)`. See docs/reports Phase 3 / SPO-31.
...
class BoTSORTReidTracklet(BaseTracklet):
