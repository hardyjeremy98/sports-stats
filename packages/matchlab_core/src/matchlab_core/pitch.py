...
minimap; x runs along the pitch length, y along the width. The 32 vertices
match the keypoint order of the roboflow-jvuqo football-field-detection
model, so model output index i corresponds to VERTICES[i] directly.

Two specs are available: SOCCER_PITCH (120x70 m, the non-physical roboflow
template yolo-pitch-local was trained on) and FIFA_PITCH (105x68 m, real
physically-consistent geometry for accurate calibrators). A pipeline config's
`pitch:` field (see matchlab_core.config.PipelineConfig) selects between them
via get_pitch(); the runner wires the result into StageContext.pitch.
...
def _soccer_vertices() -> list[tuple[float, float]]:
    ...
    this template -- stays self-consistent. Accurate calibrators use
    FIFA_PITCH instead."""
...
# these are physically consistent, so an accurate calibrator can map
