...
calibration exchange contract (docs/reference/external-calibrators-setup.md).

Mirrors matchlab_core.spotting.bridge. This module is the isolation seam — it
imports nothing model-specific (no torch, no HRNet, no PnLCalib), only the
job-manifest / homographies-JSON shapes the contract fixes. The real PnLCalib
CLI (its own sibling venv, torch-pinned, GPL code) and the permissive in-repo
reference calibrator (`matchlab_core.calib.reference_cli`) are interchangeable
behind this call — swapping `command` is the only thing that changes.
"""
...
class CalibrationBridgeError(RuntimeError):
    ...
    did not leave valid contract JSON at `out_path`. Never silently treated as
    an empty result — a frame the model genuinely could not calibrate is a
    record with `homography: null`, written by the calibrator itself, not the
    bridge inventing one."""
...
def run_calibrator(
    ...
    *,
    ...
    try:
        return [ExternalHomography.model_validate(item) for item in raw]
