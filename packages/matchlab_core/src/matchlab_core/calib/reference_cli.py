...
Implements the calibration exchange contract
(docs/reference/external-calibrators-setup.md) without any real model: it reads
a job manifest, resolves the ordered list of source-video frame indices from
its frames_dir listing, and emits one image→pitch-cm homography record per
frame — a single fixed, invertible matrix at constant confidence. It exists so
the subprocess bridge and the `pnlcalib` pipeline stage can be developed and
tested without the real PnLCalib model (its own sibling venv, torch-pinned, GPL
code, GPU).
...
# A fixed, invertible image→pitch-cm homography. Not a real calibration — just a
# well-formed matrix so the bridge/stage round-trip produces non-null
# FrameCalibrations in tests. Purely a reference-CLI implementation detail; the
# contract only fixes the *shape* of each record.
