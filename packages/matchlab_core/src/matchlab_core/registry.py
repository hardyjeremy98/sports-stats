...
if TYPE_CHECKING:
    from matchlab_core.interfaces import Stage
...
def register(kind: StageKind, name: str) -> Callable:
    ...
    Implementations self-register on import; matchlab_core.stages imports every
...
def build(kind: StageKind, name: str, params: dict) -> Stage:
    import matchlab_core.stages  # noqa: F401 — trigger registration
