...
is proved once by the `PITCHLAB_SLOW_TESTS`-gated smoke test below, which also
...
def test_dinov2_registered_alongside_osnet():
    from pitchlab_core.stages.associate.embedders import EMBEDDERS
...
def test_dinov2_module_import_does_not_eagerly_import_torch_or_timm():
    ...
    from pitchlab_core.stages.associate.embedders import dinov2 as dinov2_module
...
def test_preprocess_empty_returns_zero_batch():
    from pitchlab_core.stages.associate.embedders.dinov2 import _preprocess
...
@pytest.mark.skipif(
    ...
    reason="downloads real DINOv2 weights from the HF hub; set PITCHLAB_SLOW_TESTS=1 to run",
)
