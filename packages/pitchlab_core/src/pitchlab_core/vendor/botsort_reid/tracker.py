...
# ------------------------------------------------------------------------
#
# VENDORED AND MODIFIED for pitchlab (SPO-31). Source:
# `trackers==2.4.0` trackers/core/botsort/tracker.py (Apache-2.0, unchanged
# license). Modifications: renamed BoTSORTTracker -> BoTSORTReidTracker; adds a
# quality-gated body-appearance (ReID) cost to the FIRST association only —
# boost-only so appearance re-ranks IoU-feasible pairs but never forces/vetoes
# a match. Stable KF/state/CMC/util modules are imported unchanged from the
# installed `trackers` package. See docs/reports Phase 3 / SPO-31.
...
class BoTSORTReidTracker(BaseTracker):
    ...
    def __init__(
        ...
        self.tracks: list[BoTSORTReidTracklet] = []
    ...
    def update(
        ...
        confirmed_tracks: list[BoTSORTReidTracklet] = []
        unconfirmed_tracks: list[BoTSORTReidTracklet] = []
        lost_tracks: list[BoTSORTReidTracklet] = []
        ...
        for row, col in matched:
            ...
            if (
                ...
                track.tracker_id = BoTSORTReidTracklet.get_next_tracker_id()
    ...
    def reset(self) -> None:
        ...
        self.tracks = []
        self.frame_id = 0
        BoTSORTReidTracklet.count_id = 0
