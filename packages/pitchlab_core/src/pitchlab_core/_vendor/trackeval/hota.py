...
# (trackeval/metrics/hota.py), MIT License (see LICENSE in this directory).
...
#   1.23.5 only warns, numpy 1.26.4 raises AttributeError; this repo pins
#   numpy 2, well past the removal). Same failure class as evaluation.py's
...
class HOTA(_BaseMetric):
    ...
    def eval_sequence(self, data):
        ...
        for field in self.float_array_fields + self.integer_array_fields:
            res[field] = np.zeros((len(self.array_labels)), dtype=float)  # PATCHED: was dtype=np.float
        for field in self.float_fields:
            res[field] = 0

        # Return result quickly if tracker or gt sequence is empty
        if data['num_tracker_dets'] == 0:
            res['HOTA_FN'] = data['num_gt_dets'] * np.ones((len(self.array_labels)), dtype=float)  # PATCHED: was dtype=np.float
            res['LocA'] = np.ones((len(self.array_labels)), dtype=float)  # PATCHED: was dtype=np.float
            res['LocA(0)'] = 1.0
            return res
        if data['num_gt_dets'] == 0:
            res['HOTA_FP'] = data['num_tracker_dets'] * np.ones((len(self.array_labels)), dtype=float)  # PATCHED: was dtype=np.float
            res['LocA'] = np.ones((len(self.array_labels)), dtype=float)  # PATCHED: was dtype=np.float
            res['LocA(0)'] = 1.0
            return res
