...
# directory). Unmodified except:
#   PATCHED: relative imports flattened -- upstream lives at
#   trackeval/metrics/_base_metric.py and does `from .. import _timing` /
#   `from ..utils import TrackEvalException` (parent-package siblings); this
#   vendored copy has no `metrics` subpackage, so both become same-package
#   imports (`from . import _timing`, `from .utils import TrackEvalException`).
import numpy as np
