...
def env(tmp_path, monkeypatch):
    """Point pitchlab_server settings (data dir, DB, configs dir) at a
    private tmp tree, the same PITCHLAB_* env-var override existing tests
    use (see test_reid_labels.py's `db` fixture), so these tests can never
    touch the real repo's data/pitchlab.db or configs/datasets/*.json."""
    import pitchlab_server.db as db_module
    from pitchlab_server.settings import get_settings

    data_dir = tmp_path / "data"
    config_dir = tmp_path / "configs"
    monkeypatch.setenv("PITCHLAB_DATA_DIR", str(data_dir))
    monkeypatch.setenv("PITCHLAB_DATABASE_URL", f"sqlite:///{data_dir}/test.db")
    monkeypatch.setenv("PITCHLAB_CONFIG_DIR", str(config_dir))
...
def test_ingest_sportsmot_registers_video_and_manifest(env, tmp_path):
    ...
    from pitchlab_server.models import Video
