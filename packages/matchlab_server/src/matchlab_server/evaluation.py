...
Wraps matchlab_core.evaluation: loads the GT file pointed at by
...
from matchlab_core.attribution import match_instances
...
def evaluate_run_against_gt(
    ...
    comparison; refusals (`matchlab_core.attribution`) raise ValueError."""
    from matchlab_core.event_gt import is_event_ground_truth
    from matchlab_core.provenance import hash_evaluation_set
    from matchlab_core.schemas.run import RunManifest
    ...
    if is_event_ground_truth(gt_path):
        from matchlab_core.action_spotting_eval import evaluate_spotting_run
        from matchlab_core.event_gt import load_event_ground_truth
...
def merged_metrics(run: Run, result: dict) -> dict:
    ...
    if result.get("kind") == "action_spotting":
        from matchlab_core.action_spotting_eval import spotting_headline_metrics
...
def diff_switch_instances(
    ...
    `matchlab_core.attribution.match_instances` (the same matcher the SPO-19
