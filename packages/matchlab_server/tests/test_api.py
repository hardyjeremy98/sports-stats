...
os.environ["MATCHLAB_DATA_DIR"] = tempfile.mkdtemp(prefix="matchlab-test-")
os.environ["MATCHLAB_DATABASE_URL"] = (
    f"sqlite:///{os.environ['MATCHLAB_DATA_DIR']}/test.db"
)
os.environ["MATCHLAB_CONFIG_DIR"] = str(Path(__file__).parents[3] / "configs")
...
@pytest.fixture(scope="module")
def video_id(client):
    path = Path(os.environ["MATCHLAB_DATA_DIR"]) / "clip.mp4"
...
def test_run_diff_eval_payloads_and_switch_diff(client, video_id):
    ...
    run_dir_a = Path(os.environ["MATCHLAB_DATA_DIR"]) / "runs" / ids[0]
    ...
    run_dir_b = Path(os.environ["MATCHLAB_DATA_DIR"]) / "runs" / ids[1]
...
def test_run_diff_same_video_guard(client, video_id):
    ...
    path_b = Path(os.environ["MATCHLAB_DATA_DIR"]) / "clip_b.mp4"
