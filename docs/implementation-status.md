...
| Player-centric action spotting (B3+B4, PCBAS) | **Stage 1 measured; stage 2 built and failing** | Two-stage player-centric spotter reproducing the FOOTPASS `TAAD → DST` baseline, joined by a frozen `(9, 26, T)` fp16 contract. **Stage 1 measured on FOOTPASS VAL: micro-F1 0.3274 / macro 0.2056 against the reference TAAD arm's 0.4100 / 0.2445** — 80% of it on 60% of the training schedule (12 of 20 epochs, stopped on a wall-clock budget, still improving). We are MORE precise than the reference (0.357 vs 0.304) and less than half as sensitive; the gap is model quality, not an operating point (at matched prediction volume we recall 0.470 vs its 0.630, and a decode-threshold sweep gains at most +0.013). We beat it on 4 of 8 classes, all rare. **Stage 2 currently DEGRADES the pipeline ~3x: TAAD→DST scores 0.1188** against the reference's 0.7186. Two bugs found and fixed (encoder fed absolute video frames while the decoder used window-local timestamps, costing 3.4x; the one-hot frame channel disagreeing with the positional encoding); one hypothesis measured and REFUTED (random-offset window sampling changed nothing). DST's confidence is near-uninformative — restricted to its top 826 predictions precision is still 0.119. Not ruled out: ~5x less training exposure than the reference, and weak stage-1 input (30% event recall vs 63%). An oracle-logits experiment scored 0.9104, but that comparison was **CONFOUNDED AND IS RETRACTED**: both arms had a 3h budget, the oracle arm fitted 15 epochs and the real arm only 10, and through the matched epochs the two are indistinguishable (the real arm is ahead on epochs 3-8). The oracle advantage appears entirely in the 5 epochs the real arm never ran. **Stage-1 quality is NOT established as the binding constraint.** What IS measured: our DST raises event coverage from 0.405 to 0.682 within +/-200 frames (recall 0.632 at delta=200, equal to the reference TAAD) but smears events temporally, so the failure is specifically an under-converged timestamp head. Audit also found the DST recipe diverges from the reference on LR decay, warmup fraction, weight decay, label smoothing and checkpoint selection. Three reproduction gates pass exactly (metric, ingest, training anchors). ADR 008 roster remap is lossless on VAL (ground-truth roles only). Identity contract: [ADR 008](decisions/008-role-slots-are-not-roster-slots.md). See [Phase 0](reports/2026-07-27-pcbas-phase0-ingest.md), [Phase 1 build](reports/2026-07-27-pcbas-phase1-build.md), [Phase 1 gate](reports/2026-07-27-pcbas-phase1-gate.md), [DST investigation](reports/2026-07-28-pcbas-dst-investigation.md). | `matchlab_core/pcbas/{schema,events,eval,logits,decode,action_head,denoiser}.py`, `matchlab_train/datasets/footpass_{pcbas,video,clips,windows}.py`, `matchlab_train/experiments/pcbas_*.py`, `matchlab-train footpass-stats` |
