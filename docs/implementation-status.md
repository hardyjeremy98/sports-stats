...
| Player-centric action spotting (B3+B4, PCBAS) | **Designed, not built** | Architecture and Phase 0–1 plan for reproducing the FOOTPASS `TAAD → DST` baseline: a shared-backbone video model pooling per-player features by `roi_align` off a stride-8 map, then an encoder-decoder Transformer translating a 750-frame window of per-slot logits into an event list. Joined by a frozen `(9, 26, T)` fp16 contract. **No code yet.** FOOTPASS tactical data (TRAIN/VAL/CHALLENGE, 9.9 GB HDF5) and VAL video (352×640) are on disk; TRAIN video downloading. Verified during design: video zips are AES (needs `7z`), **one mp4 per match with frame indices continuous across halves**, 640×352 @ 25 fps, and ROI scaling `/3.0`, `/3.068181` lands on centred players. Critical path is **not** the visual model — it is side/half determination (`left_to_right` inverts at half time, 17/18 shirts), role assignment (11 of 13 roles used per side, not a 13-permutation) and **slot stability** (tracklet gaps reach 482 frames = 64% of a DST window). Reproduction bars are the reference's own VAL scores, reproduced locally from its shipped predictions: micro-F1 **41.0** (TAAD) / **71.86** (TAAD+DST); the widely-quoted 46.41 is a challenge-split figure not reproducible from the repo. Identity contract governed by [ADR 008](decisions/008-role-slots-are-not-roster-slots.md) — role slots are **not** roster slots. See [design](superpowers/specs/2026-07-27-player-centric-action-spotting-design.md) + [plan](superpowers/plans/2026-07-27-player-centric-action-spotting.md). | — (no modules yet) |
