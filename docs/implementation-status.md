...
| Camera calibration | Implemented | Static, Roboflow keypoint, local YOLO (`yolo-pitch-local`, 32-kp YOLOv8x-pose — fragile on pans/occlusion), and **PnLCalib** (`pnlcalib`, HRNet keypoints+lines + points-and-lines optimization, external sibling venv via subprocess) variants. PnLCalib qualitatively far more robust on broadcast clips (amateur-footage accuracy unverified). Shared EMA/carry smoothing (`calibrate/smoothing.py`); external exchange in `calib/bridge.py` + reference CLI. | `matchlab_core/stages/calibrate/`, `matchlab_core/calib/`, [`external-calibrators setup`](reference/external-calibrators-setup.md) |
