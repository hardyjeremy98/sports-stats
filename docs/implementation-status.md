...
| Player-centric action spotting (B3+B4, PCBAS) | **Built, not trained** | Two-stage player-centric spotter reproducing the FOOTPASS `TAAD → DST` baseline: a shared-backbone X3D-S video model pooling per-player features by `roi_align` off a stride-8 map, then an encoder-decoder Transformer translating a 750-frame window of per-slot logits into an ordered event list, joined by a frozen `(9, 26, T)` fp16 contract. **Every component is implemented and tested; no model is trained yet and no reproduction is claimed.** Two gates pass: our metric reproduces the reference's aggregated per-class table exactly on its own shipped VAL predictions (TAAD micro-F1 **0.4100**, TAAD+DST **0.7186**, TP\_noBB 33 vs 390), and the ingest reproduces the published event totals exactly (VAL 6,070 / TRAIN 91,327), with VAL's per-class counts matching the reference's own GT JSON on every class. Measured end to end on real footage: 2.2 clip/s training, 0.04 s per clip read, 0.32 s per inference window, 6.2 GiB peak at batch 1 (batch 2 OOMs on a 16 GiB card, so micro-batch 1 × accum 48 holds the reference's effective batch of 48). Both DST encoders exist — `flat` (reference) and `attn` (PAVE-style per-player attention). Measured corrections to the design: **all 26 slots appear in TRAIN** (the 11-of-13-roles finding is VAL-specific), and TRAIN has **285** tackles / **1,097** shots, not the planned ~390 / ~1,000. Critical path remains side/half determination, role assignment and slot stability, none of which are built. Identity contract governed by [ADR 008](decisions/008-role-slots-are-not-roster-slots.md) — role slots are **not** roster slots. See [Phase 0](reports/2026-07-27-pcbas-phase0-ingest.md), [Phase 1 build](reports/2026-07-27-pcbas-phase1-build.md), [design](superpowers/specs/2026-07-27-player-centric-action-spotting-design.md), [plan](superpowers/plans/2026-07-27-player-centric-action-spotting.md). | `matchlab_core/pcbas/{schema,events,eval,logits,decode,action_head,denoiser}.py`, `matchlab_train/datasets/footpass_{pcbas,video,clips,windows}.py`, `matchlab_train/experiments/pcbas_*.py`, `matchlab-train footpass-stats` |
