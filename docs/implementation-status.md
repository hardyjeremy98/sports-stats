...
| Player-centric action spotting (B3+B4, PCBAS) | **Stage 1 measured; stage 2 built, untrained** | Two-stage player-centric spotter reproducing the FOOTPASS `TAAD → DST` baseline, joined by a frozen `(9, 26, T)` fp16 contract. **Stage 1 (X3D-S + FPN + `roi_align`) is trained and measured on FOOTPASS VAL: micro-F1 **0.3274** / macro-F1 0.2056 against the reference TAAD arm's 0.4100 / 0.2445** — 80% of it on 60% of the training schedule (12 of 20 epochs, stopped on a wall-clock budget, still improving at the last epoch). We are MORE precise than the reference (0.357 vs 0.304) and less than half as sensitive (recall 0.303 vs 0.630); the whole gap is recall, and it sits in the decode `argmax` — zero of 5,146 decoded events were removed by the τ=0.15 floor. We beat the reference on 4 of 8 classes, all rare ones. Stage 2 (DST encoder-decoder, both `flat` and PAVE-style `attn` encoders) is implemented and smoke-tested on real logits (45.5M params, 0.11 s/step, 1.4 GiB) but **not trained** — it needs stage-1 logits over all 96 TRAIN halves first. Three gates pass exactly: the metric reproduces the reference's per-class table on its own shipped VAL predictions (0.4100 / 0.7186), the ingest reproduces the published event totals (6,070 / 91,327), and our training anchors match its shipped sample list on both splits (74,377 / 5,008). Measured findings: the ADR 008 roster remap is **lossless** on VAL (`by_slot` and `by_shirt` identical, 0 unknown shirts); visual-only off-screen recovery is negligible (9 of 1,062, reference 33) against DST's 390; validation loss is the **wrong** checkpoint criterion here and would have cost 16% relative. Identity contract governed by [ADR 008](decisions/008-role-slots-are-not-roster-slots.md). See [Phase 0](reports/2026-07-27-pcbas-phase0-ingest.md), [Phase 1 build](reports/2026-07-27-pcbas-phase1-build.md), [Phase 1 gate](reports/2026-07-27-pcbas-phase1-gate.md), [design](superpowers/specs/2026-07-27-player-centric-action-spotting-design.md). | `matchlab_core/pcbas/{schema,events,eval,logits,decode,action_head,denoiser}.py`, `matchlab_train/datasets/footpass_{pcbas,video,clips,windows}.py`, `matchlab_train/experiments/pcbas_*.py`, `matchlab-train footpass-stats` |
