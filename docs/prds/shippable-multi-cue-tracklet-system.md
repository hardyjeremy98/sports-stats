...
2. **Pose — adopt.** RTMPose (Apache-2.0, MMPose). Wire it in as the keypoint front-end; no
   training expected.
3. **ReID / appearance embedder — train (licensing-forced).** Real person-ReID datasets are
   research-only, so train a shippable embedder on **synthetic generalizable-ReID data**
   (RandPerson, UnrealPerson) with the option of self-supervised pretraining later when owned
   footage exists. Reuse the existing offline associator's quality-gated crop sampler and
   threshold-calibration harness rather than rebuilding them.
4. **Association head — train (licensing-forced).** The SPO-34-selected head, retrained on
   **permissive/synthetic tracking data** (candidate sources: MOTSynth-class synthetic MOT;
   pseudo-labels generated by running a strong *dev-only* reference tracker over a permissive
   video corpus — association-centric / link-prediction training is designed to consume
   tracker-states, which makes pseudo-labels viable). SportsMOT/DanceTrack are non-commercial
   and are used for **evaluation only**, never as shippable training data.
...
- **Bar A — benchmark-parity, IN SCOPE.** The shippable in-house stack lands **within a
  pre-registered threshold** of the non-shippable SOTA reference on shared held-out public
  benchmarks (SportsMOT + SoccerNet), across the full metric stack with **tracklet purity /
  mixed-track duration primary**, HOTA/AssA/IDF1 secondary, and detection recall / crop yield
...
7. As a researcher, I want a shippable appearance embedder trained on synthetic
   generalizable-ReID data, so that the appearance cue is available without depending on
...
**The largest risk is the association training data,** not code. Every code component is
permissive; the walls are all training-data licensing. ReID has a credible synthetic path
(RandPerson/UnrealPerson); the association head does not yet have a proven permissive source,
so its data-sourcing issue carries a pre-registered fallback (if no source reaches Bar A, the
shippable-association question escalates before further build investment).
